...
"""Menu():
    ...
    print("3-Emitir prod. en orden Vendidos.")
    print("4-Emitir prod. en orden Disponibles.")
    print("5-Modificar Precio.")
    print("6-Incrementar 10%.")
    print("7-Buscador.")
    print("8-Imprimir Stock.")
    print("0-Salir.")
    opc = int(input("Opción: "))
    return opc"""

# Cat 1: Piletas Cat 2: Pelopinchos Cat 3: Mantenimiento

class productos:
    def __init__(self, cod, art, cat, mar, des, col, pre, dis):
        self.cod = cod #Codigo
        self.art = art #Nom articulo
        self.cat = cat #Categoria= Cat 1: Piletas Cat 2: Pelopinchos Cat 3: Mantenimiento
        self.mar = mar #Marca del prod.
        self.des = des #Descripción
        self.pre = pre #Precios
        self.est = dis #Disponible 1, No disponible 2.
    def __repr__(self):
        return "Cod:{0}, {1}, Ctg:{2}, $:{3}, Est:{4}\n".format(self.cod, self.nom, self.cat, self.pre, self.est) #Devuelve en pantalla
...
with open("Productos.txt","r") as arch:
    for linea in arch:
        a=linea.split(",")
        lstProduc.append(productos(int(a[0]), a[1], int(a[2]), int(a[3]), int(a[4])))
...
def EmitirOrd1(lstProduc):
    ...
    with open ("Orden alfabetico.txt","w+") as arch:
        for i in range(0,len(lstProduc)):
            a=lstProduc[i]
            arch.write("Cod:{0}, {1}, Ctg:{2}, $:{3}, Est:{4}\n".format(a.cod, a.nom, a.cat, a.pre, a.est))
